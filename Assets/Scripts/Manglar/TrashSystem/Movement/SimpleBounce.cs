...
public class SimpleBounce : MonoBehaviour
{
    ...
    public float floatSpeed = 5f; // Velocidad de subida hacia la superficie
    ...
    void Update()
    {
        if (isRising) {
            ...
            float elapsedTime = (2f - startTime) * floatSpeed;
            ...
            if (t >= 1f) {
                ...
                isRising = false;
                startTime = 1f; // Reiniciar el tiempo para el rebote
            }
        } else if (hasReachedSurface) {
            // Controlar el rebote usando la curva de rebote
            float elapsedTime = (2f - startTime);
            float t = Mathf.Clamp01(elapsedTime / bounceDuration); // Normalizar el tiempo de la animación entre 0 y 1

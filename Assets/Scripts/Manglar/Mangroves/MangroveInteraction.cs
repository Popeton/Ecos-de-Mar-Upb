...
public class MangroveInteraction : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            Debug.LogError("VRInteractionHandler no encontrado. Asegúrate de que está en la escena inicial y tiene DontDestroyOnLoad.");
            enabled = false;
            return;
        }
    ...
    {
        if (interactionHandler != null && isSubscribed)
        {
            interactionHandler.OnInteractionStarted -= HandleMangroveInteraction;
            isSubscribed = false;
        }
